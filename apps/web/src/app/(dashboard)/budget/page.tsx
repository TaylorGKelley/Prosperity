			...
			<div className='px-12'>
				...
				<main className='px-10 flex flex-col gap-6'>
					...
					{data.categories.map((category) => (
						<div key={category.id} className='bg-white rounded-2xl shadow flex gap-4 px-8 py-6'>
							<CategoryIcon
								icon={category.icon.toLowerCase().replace('_', '-') as CategoryIconKey}
								color={category.color.toLowerCase().replace('_', '-') as CategoryColorKey}
							/>
							<div className='flex flex-col gap-2 w-full'>
								...
								<div className='flex gap-4 items-center w-full'>
									<p className='font-normal text-sm text-gray-500 text-nowrap'>
										{Math.floor((category.totalSpent / category.amount) * 100)}% used
									</p>
					...
					{data.savingGoals.map((savingGoal) => (
						<div key={savingGoal.id} className='bg-white rounded-2xl shadow flex gap-4 px-8 py-6'>
							<CategoryIcon
								icon={savingGoal.icon.toLowerCase().replace('_', '-') as CategoryIconKey}
								color={savingGoal.color.toLowerCase().replace('_', '-') as CategoryColorKey}
							/>
					...
					<ul className='flex flex-col gap-9'>
						{data.accounts.map((account) => (
							<li key={account.id} className='flex gap-5 items-center'>
								<CategoryIcon
									icon='piggy-bank'
									color={account.color.toLowerCase().replace('_', '-') as CategoryColorKey}
								/>
								<div>
				...
				<section className='flex flex-col gap-9'>
					...
					<ul className='flex flex-col gap-9'>
						{data.budgets.map((budget) => (
							<li key={budget.id} className='grid grid-cols-[auto_1fr_auto] gap-5 items-center'>
								<CategoryIcon
									icon={'wallet'}
									color={budget.color.toLowerCase().replace('_', '-') as CategoryColorKey}
								/>
								<div>
