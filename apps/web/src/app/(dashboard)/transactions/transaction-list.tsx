...
import getAllTransactions, {
  type GetAllTransactionsResult,
} from "@/actions/transaction/getAll";
  ...
  return (
    <ul className="grid gap-6 pb-8">
      {transactions.map((transaction) => (
        ...
        >
          <div className="rounded-md bg-blue-100 size-10">
            {transaction.categoryId}
          </div>
          <div className="grid">
            <h5>{transaction.description}</h5>
            <p className="opacity-75 text-sm">
              {Format.date(transaction.date).dateOnly}
            </p>
          </div>
          ...
          <p className="font-bold text-lg text-right flex-grow">
            {transaction.amount}
          </p>
