...
export default async function Transactions({ params }: TransactionsPageProps) {
	...
	const query = graphClient.query<TransactionPageQuery, TransactionPageQueryVariables>({
		query: TRANSACTION_PAGE_QUERY,
		variables: {
			monthDate: selectedDate,
			budgetId: selectedBudgetId as UUID | undefined,
				...
				<main className='px-10 flex flex-col gap-6 pb-12'>
					...
					<section className='flex flex-col gap-8 w-full '>
						<Suspense
							...
							<TransactionList transactionsQuery={query.then(({ data }) => data.transactions)} />
						</Suspense>
						{/* Pending Heading */}
						...
						{data.transactions.items
							.filter((transaction) => transaction.status === TransactionStatusEnum.Pending)
							.map((transaction) => (
						...
						{data.transactions.items
							.filter((transaction) => transaction.status === TransactionStatusEnum.Posted)
							.map((transaction) => (
								<TransactionCard key={transaction.id} transaction={transaction} />
							))}
					</section>
				...
				<section className='flex flex-col gap-9'>
					...
					<ul className='flex flex-col gap-9'>
						{data.categories.map((category) => (
							<li key={category.id} className='grid grid-cols-[auto_1fr_auto] gap-5 items-center'>
								<CategoryIcon
								...
								<p
									className={cn('text-sm bg-gray-100 px-1.5 py-0.75 rounded-md', {
										'bg-red-100 text-red-500': category.amount - category.totalSpent < 0,
									})}>
